...
module.exports = {
	...
	async execute(interaction, noblox, admin) {
        ...
        const isReady = db.ref("szeebe/alapha-universe-docs-signature/Players/getPlayerInfo");
                ...
                async function checkPlayersSingleParam(PlayerType, userData) {//(userData) {
                    ...
                    for (let i = 0; i < userData.length; i++) {
                        setTimeout(async function timer() {
                            try {
                                const playerInfo = await noblox[PlayerType]({
                                    userId: Number(userData[i].childData.userId)
                            });
